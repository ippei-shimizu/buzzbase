...
- 不要なのは「コードと同じことを言うだけのコメント」「履歴メモ」など。意味のあるドキュメンテーションコメントは残す
...
- コード規約・設計判断としてチーム全体に効く学びは、**同じ指摘が2回目に出た時点でのみ** CLAUDE.md / 各リポジトリの `.claude/rules/*.md` への追加をdiffで提案する。ユーザー承認前に書き込まない
