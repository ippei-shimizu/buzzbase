...
インラインコメントは `claude-code-action` が標準で持つ機能で、追加のツール許可設定は不要（`classify_inline_comments` の既定値は `true`）。ただし `permissions` は upstream のリファレンス構成に合わせて `pull-requests: write` にしておく。`read` のままだとインラインコメントの投稿に失敗する可能性がある。

インラインコメントが1件も返らずまとめコメントだけだった場合は、依頼本文の指示の弱さか、この permissions を疑う（「8. レビュー内容の取得」参照）。
...
- **インラインコメントが0件で通常コメントだけが返ってきた場合**は、通常コメントの内容でそのまま対応を進めつつ、ワークフローの `permissions` が `pull-requests: write` になっているかを確認し、結果を最終報告に明記する
