...
description: PRのURLまたは番号を引数に受け取り、差分を分析して観点付きの `@claude` コードレビュー依頼コメントをPRに投稿し、差分の該当行へのインラインコメントで指摘を返してもらう。レビュー結果が返ってくるまで待機したうえで、指摘内容を確認して対応要否を判断し、修正が完了するまで自動で行う。「PRにレビュー依頼して」「@claudeでレビューお願いして」「request-claude-review」などで起動する。
...
指摘は**差分の該当行に紐づいたインラインコメント**として返るように依頼する。どのコードのどの行の話なのかがコメント自体に埋め込まれるため、対応時の特定が正確になり、GitHub上でも解決済み/未解決を追える。
...
- **返信でコミットハッシュに触れるときは、必ずコミットの URL をリンクで添える**（同上）
...
起動条件は `issue_comment` の `created` イベントで、`github.event.comment.body` に `@claude` が含まれること。したがって PR へのコメント投稿（`gh pr comment`）で起動する。レビュー結果は、ワークフロー完了後に同じ PR へ `github-actions` bot による**差分行へのインラインコメント**として投稿される。行に紐づけられない全体所感のみ通常コメントとして投稿される。

### インラインコメントの前提条件

インラインコメント用の MCP サーバーは、`allowedTools` に指定されたときだけインストールされる。依頼本文で指示しても、ワークフロー側が下記2点を満たしていなければ投稿手段が無く、まとめコメントにフォールバックする。
...
`--allowedTools` はベースツール（Glob / Grep / Read 等）を置き換えず追加される。

投稿されたインラインコメントは既定で一度バッファされ、Haiku による分類（実レビューか検証用の試し投稿か）を通ってから投稿される（`classify_inline_comments` の既定値が `true`）。依頼本文で `confirmed: true` を付けて投稿するよう指示すれば、この分類を経ずに即時投稿される。

インラインコメントが1件も返らずまとめコメントだけだった場合は、まずこの2つの設定を疑う（「8. レビュー内容の取得」参照）。
...
- 指摘は必ず**該当ファイルの該当行へのインラインコメント**として投稿してください。1件のまとめコメントに指摘を列挙する形式にはしないでください
- 投稿には `mcp__github_inline_comment__create_inline_comment` を `confirmed: true` で使ってください
- 各インラインコメントには「何が問題か」「なぜ問題か」「どう直すか」を含め、修正案が示せる場合は suggestion 形式のコードブロックを添えてください
- 特定の行に紐づけられない全体所感・設計レベルの懸念のみ、通常のコメントとして投稿してください
...
- 「指摘の投稿方法」セクションと末尾のマージ禁止の一文は必ず入れる
...
ワークフロー完了後、`REQUEST_COMMENT_AT` より後に付いた新規コメント・レビューを取得する。**インラインコメントが本体**なので、まずそこから読む。

```bash
# インラインコメント（本体）。path / line / diff_hunk を必ず控える
gh api repos/"$REPO"/pulls/"$PR_NUMBER"/comments --paginate --jq \
  --arg since "$REQUEST_COMMENT_AT" \
  '[.[] | select(.created_at > $since)] | .[] | {id, path, line, start_line, diff_hunk, body, html_url}'

# レビュー本体のサマリ（インラインコメントを束ねる親）
gh api repos/"$REPO"/pulls/"$PR_NUMBER"/reviews --jq \
  --arg since "$REQUEST_COMMENT_AT" \
  '[.[] | select(.submitted_at > $since)] | .[] | {user: .user.login, state, body}'
...
インラインコメント1件を指摘1件として扱い（通常コメントに複数の指摘が含まれる場合は指摘ごとに分解し）、それぞれについて以下のいずれかに分類する。
...
- 指摘1件 = 1コミット。コミットメッセージは日本語・`[Type]: [説明]` 形式（モノレポの規約）で、どの指摘に対応したかが単独で読み取れるようにする（対象ファイル名を含めると対応関係が追いやすい）
...
| 分類 | 書くこと |
| ---- | ---- |
| 対応した | 「対応しました（コミットへのリンク）」＋ 何をどう直したかを1〜2文 |
...
- **コミットハッシュを書くときは必ずコミットの URL をリンクにして添える**。ハッシュだけでは GitHub 上でクリックして差分に飛べず、別リポジトリのコミット（front / mobile 対の修正など）はどのリポジトリの話か判別できないため
  - 形式: `[<短縮ハッシュ>](https://github.com/<owner>/<repo>/commit/<フルハッシュ>)`
  - 別リポジトリのコミットに触れるときは、リンクに加えてリポジトリ名も文中に書く（例: front 側（`ippei-shimizu/buzzbase_front`）も修正しました）
  - 短縮ハッシュとフルハッシュは `git log -1 --format='%h %H'` で取る。URL には必ずフルハッシュを使う
