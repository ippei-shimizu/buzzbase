...
- **返信はすべて日本語で書く**。セッション上のユーザーへの返信・途中経過・最終報告、PR description、GitHub に投稿するコメント・レビュースレッドへの返信（`request-claude-review-cloud` の手順に従う部分も含む）が対象。コードの識別子・コマンド・エラーメッセージの引用は原文のままでよい
