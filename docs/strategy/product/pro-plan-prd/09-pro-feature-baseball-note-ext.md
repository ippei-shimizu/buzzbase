...
**作成日**: 2026-05-12
**最終更新**: 2026-06-27（練習への紐付け・IA配置を追記）
**ステータス**: ドラフト（実装直前に詳細化）
...
- アップロード上限は**点数制限**を採用（合計サイズ制限は不採用）
  - 理由: R2 でストレージが激安、かつ無料は480p/30秒/30日削除で1点が小さくサイズで縛る実利が薄い。点数の方が分かりやすく Pro 転換レバーとして強い
- **無料**: 月3点（画像・動画合算）/ 動画30秒 / 480p / 30日で自動削除 / 画像5MB
- **Pro**: 無制限 / 動画60秒 / 1080p / 永続 / 画像10MB
