...
**作成日**: 2026-05-12
**最終更新**: 2026-08-11（release/pro-202605 の実装を正として機能一覧・数量制限・admin 強制 Pro を実態に同期）
...
│  - Role: admin / user                      │
│  - Plan: subscription.status（下表の7状態）│
├───────────────────────────────────────────┤
...
> Pro 期間中に作成したデータ（4個目以上の練習メニュー、31日以上前の草機能の記録等）は閲覧・操作不可になる（データ自体は DB に残る）。
...
> 当初は admin 権限で「強制 Pro モード」に入れる（development 環境のみ）想定だったが未実装。
> 開発時の Pro 動作確認は subscription レコードを直接書き換えて行う。
...
| 動画・画像（過去アップロード分） | 表示可 | 表示可 | 表示可（保管期間で差をつけない） |
...
- 試合記録、基本成績集計、グループ機能（1つまで所属）、ランキング
- 計算ツール、野球ノート（基本）
- 素振りカウンター（基本）
- 練習記録（基本）
- 草機能（直近30日）
- 個人の期間目標（月次/週次/年間を合算2つまで）
- 自主練スケジュール（無制限）
...
- 広告非表示
- シーズン跨ぎ成績推移グラフ
- 草機能（全期間）／カレンダー俯瞰の全期間閲覧
- 練習メニュー無制限、メニューセット無制限、週の練習プランの翌週コピー
- メニューごとの推移詳細
- 動画・画像アップロード無制限（長尺・高解像度・大サイズも可）
- 個人の期間目標無制限、カスタム期間目標、自由指標（手動更新）の目標
- シーズン目標、大会目標
- カスタム通知メッセージ
- 詳細コンディションログ
- 課題テーマ無制限、相関インサイト
- 振り返りテンプレの自作無制限、週次/月次レポートの詳細
- 野球ノートのタグ付け、複数試合記録の紐付け、複数課題の紐付け
- 素振りカウンターのインターバル自由設定・バイブレーション・バックグラウンド継続
- グループの作成・参加を無制限に
- 打撃の詳細分析（方向別／カウント別／球種別／対戦投手別の打率）

機能キーの正はコード（`back/app/models/concerns/entitlement.rb`）。
初期案の「メディアの長期保管」「自主練スケジュール無制限」「高度な目標トラッキング」は、
それぞれ他キーへの統合・無料開放・スコープ整理により廃止した。
...
| 動画の長さ・解像度 | 30秒 / 長辺480px | 180秒 / 長辺1280px |
| 画像サイズ | 5MB まで | 10MB まで |
| メディア保管期間 | 永続（無料/Pro で差をつけない） | 永続 |
| 自主練スケジュール | 無制限 | 無制限 |
| 個人の期間目標（月次/週次/年間） | 合算2つまで | 無制限 |
| カスタム期間の個人目標 | 利用不可 | 利用可 |
| シーズン目標 / 大会目標 | 利用不可 | 利用可 |
| 課題テーマ（取組中） | 2つまで | 無制限 |
| 振り返りテンプレの自作 | 1つまで | 無制限 |
| グループ所属（作成+参加） | 1つまで | 無制限 |
| 草機能の表示期間 | 直近30日 | 全期間 |
...
### admin の強制 Pro モード（開発時）— 未実装

- 「development 環境かつ admin なら自動的に Pro 扱い」は実装しなかった
- Pro 判定（`Subscription#pro_active?`）に環境・ロール依存の分岐は入っていない
- 開発時の確認は subscription レコードの status / expires_at を直接書き換えて行う
...
| 用途 | 要件 |
|----|----|
| Pro 状態の取得 | 認証済みユーザーの現在の Pro 状態（status / expires_at / platform / 期限判定済みの pro_active）と、保有している Entitlement キーの一覧をまとめて返す |
| Pro 状態の手動同期 | クライアントから明示的に RevenueCat と同期する |
| Entitlement リスト | 全 Entitlement キーごとの利用可否を返す |
| Web 加入・変更・解約 | Web 決済の Checkout 開始、プラン変更、解約を受け付ける |
| 解約理由アンケート | 解約時のアンケート回答を受け付ける |
| Webhook 受信 | RevenueCat / Stripe からのイベントを受信して状態更新 |
| Feature Flag 取得 | 公開許可された Flipper flag の有効/無効をクライアントに返す |

「特定機能のアクセス権だけを返す単発チェック API」は作らない。Pro 状態の取得が保有キー一覧を
含むため、クライアントは1回の取得で全機能を判定できる。
...
| 状況 | ユーザーへの挙動 |
|----|----|
| Pro 状態の取得失敗 | デフォルト（無料扱い）で動作、再試行ボタン表示。ただし加入状態や課金内容を提示する画面では「無料」と断定せず、取得失敗として区別する |
| Pro 状態の判定が未確定 | 確定するまでロック UI を描画しない（加入者にロックが一瞬見えるのを防ぐ） |
| Webhook 同期遅延 | 「同期更新」ボタンで強制取得可能 |
| 二重課金検出 | 加入済みステータスでブロック、サポート誘導 |
| 決済プロバイダ障害 | 直近に取得した Pro 状態を一定時間キャッシュし、その間は判定を維持する（永続キャッシュは持たない） |
