...
**作成日**: 2026-05-12
**最終更新**: 2026-06-27（完了時自動保存・無料/Pro境界の再定義・配置を確定）
**ステータス**: ドラフト（実装直前に詳細化）
...
設計検討で確定。以降の「機能要件」「Pro機能」「UI 仕様」と差異がある場合は**本セクションを優先**する。

### 配置・起動

- ホーム > 活動タブ クイック記録「⚾ 素振りを始める」から起動。設定 → 実行 → 完了 の3画面
- **mobile 限定**（リアルタイム計測のため。Web 版は提供しない）
...
### 無料 / Pro 境界（実装確定: 2026-07-27）

- **本数は無料で無制限**（10〜2,000本）
- **インターバルは無料で5〜10秒のみ**。5秒未満・10秒超のカスタム設定は Pro（`shadow_swing_custom_interval`）
- **バイブレーションは Pro 限定**（`shadow_swing_vibration`）。無料は音のみ
- **Pro**: カスタムインターバル / バイブレーション設定 / バックグラウンド継続実行 / カスタム音（Phase 2）/ 累計本数30日以上の表示
