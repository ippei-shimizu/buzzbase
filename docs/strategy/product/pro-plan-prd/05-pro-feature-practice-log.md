...
**作成日**: 2026-05-12
**最終更新**: 2026-06-27（IA配置・野球ノートとの紐付け方針を追記）
**ステータス**: ドラフト（実装直前に詳細化）
...
## ⚠️ IA / 体験方針（2026-06-27 追記）

- 配置: 練習記録は **ホーム > 活動セグメント**の「クイック記録」に置く（素振り / お気に入りワンタップ / 練習を記録 / コンディションの4導線）
- 記録の住み分け: お気に入りワンタップ＝高速版、`📝練習を記録`＝フル版、素振りカウンター＝専用計測（完了時に練習ログ自動生成）
- 野球ノートとの関係: 練習記録（定量・量）と野球ノート（定性・感想/動画）は**別データのまま緩く紐付ける**。練習記録の**保存直後にノート誘導**し、日単位（任意でメニュー単位）で紐付ける（モデルA）。詳細は設計 §4〜§5
...
- `📝練習を記録 → メニュー選択 → 量入力 → 保存 → 「続けて記録?」→ ノート誘導（モデルA）`
- 量入力: 既定値プリセット・単位ラベル付き・日付（過去遡及可）・メモ（任意）・写真（Pro/月3点）
- 1日複数メニューは「お気に入りワンタップ」or 繰り返し記録でカバー（**一括入力フォームは作らない**）

### コンディション入力（Pro限定・1日1回 upsert）
...
- `condition_logs.injuries`: `[{part, memo}]`（severity は持たない）
