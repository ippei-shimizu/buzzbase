...
- **見出しの直下にスライドショーを置く。** Pro でできることを 1 枚ずつ横スクロールで見せる。トリガーに対応するスライドを先頭へ並べ替え、自動送りはユーザーがスワイプした時点で止める
  - 方向別 / カウント別 / コース別は実際の分析 UI をそのまま見せる。方向別は無料でも同じ集計 API を参照できるため、本人の打球データがあればそれを出し、無ければサンプル表記を付ける
  - シーズン推移と広告非表示は実データを持てないため専用のイラストを描く（`PaywallSlideArt`）
  - 撮り下ろし画像は用意しない。記録アプリでは「自分の成績がこう見える」図が最も強い証明になる
...
| 機能紹介 | 見出し直下にスライドショーを置く | 文字だけの訴求では「何ができるか」が伝わらない。撮り下ろし画像は用意せず、実際の分析 UI と本人データを見せる |
