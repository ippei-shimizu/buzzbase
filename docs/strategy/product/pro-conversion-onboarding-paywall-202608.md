...
| ステップ | 内容 |
| ---- | ---- |
| value（価値画面） | トリガー機能の見出し + 図（本人データ、無ければサンプル）、同系統の機能ブロック最大3件、全機能一覧のアコーディオン、FAQ 4項目 |
...
| Paywall の共有部品 | `mobile/components/pro/paywall/`（`PaywallFlow` / `PaywallHeroVisual` / `paywallContent` / `usePaywallPurchase` / `usePaywallSteps`） |
